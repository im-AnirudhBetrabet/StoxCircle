...
export default function CohortView({ groupId, cohortId, onBack, currentUserRole, active_cohorts }) {
  ...
  const handleRecordFunds = async () => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const response = await fetch(`${import.meta.env.VITE_SERVER_URL}/ledger`, {
        method: 'POST',
        headers: { 
      ...
      <div className="responsive-main-grid">
        ...
        <div>
          ...
          <div className="glass-panel mb-8" style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div style={{ flex: 1 }}>
              {data.contributions.map(member => (
                <div key={member.id} className="flex-between mb-2" style={{ paddingBottom: '8px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <div className="modal-overlay active" onClick={(e) => e.target.classList.contains('modal-overlay') && setSelectedStock(null)}>
          <div className="glass-panel modal-content" style={{ maxWidth: '600px', width: '100%' }}>
            ...
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '24px' }}>
                ...
                <button className="glass-button" style={{ width: '100%', marginTop: '0.5rem'}} onClick={async () => {
